...
public class UserService {
    ...
    @Transactional
    public void updateWithdraw(String id, UpdateWithdrawRequestDto dto) {

        User user = userRepository.findByUserEmailOrElseThrow(id);
        if (!dto.equals("y")) {

            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "탈퇴를 원하시면 y를 입력해주세요.");
        }
        user.updateWithdraw(dto.getWithdrawYn());
        userRepository.save(user);
    }
